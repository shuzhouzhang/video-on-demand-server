...
void pollSession(evutil_socket_t, short, void *argument) {
    ...
    if (state.active && state.future.wait_for(std::chrono::milliseconds(0)) ==
        ...
        if (result.success) {
            ...
            const bool retry = result.error.rfind("retryable:", 0) == 0 ||
                               attempt < state.maxAttempts;
            state.channel->reject(state.deliveryTag, retry ? AMQP::requeue : 0);
...
RabbitMqConsumer::RabbitMqConsumer(biteconfig::RabbitMqSettings settings,
                                   std::string exchange, std::string routingKey,
                                   std::string queue,
                                   RabbitMessageHandler handler)
    : settings_(std::move(settings)), exchange_(std::move(exchange)),
      routingKey_(std::move(routingKey)), queue_(std::move(queue)),
...
void RabbitMqConsumer::run() {
    ...
    while (!stopped_.load()) {
        ...
        AMQP::TcpConnection connection(
            &eventHandler,
            AMQP::Address(
                settings_.host, static_cast<std::uint16_t>(settings_.port),
                AMQP::Login(settings_.user, password), settings_.virtualHost));
        ...
        channel.setQos(1);
        channel.consume(queue_).onReceived([&](const AMQP::Message &message,
            ...
            state.future = std::async(
                ...
                [handler = state.handler, consumed = std::move(consumed)] {
                    ...
                    if (!result.success &&
                        result.error.rfind("retryable:", 0) == 0)
        ...
        EventPtr poller(
            event_new(loop.get(), -1, EV_PERSIST, pollSession, &state),
            &event_free);
