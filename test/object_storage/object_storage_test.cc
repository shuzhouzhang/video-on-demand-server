...
int main() {
    ...
    bitestorage::StoredObject second;
    ok &= expect(!storage->put("..", "outside.mp4", payload, second, error),
                 "reject parent directory on upload");
    ok &= expect(storage->put("source", "../unsafe name.mp4", "second", second, error) &&
                     second.locator != stored.locator,
                 "same-name uploads receive distinct object locators");
