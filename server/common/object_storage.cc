...
public:
    ...
    bool put(const std::string& directory,
        ...
        // 随机命名和排他创建共同防止并发上传覆盖已有文件。
        std::string unique;
        if (!bitesession::generateSessionToken(unique, error)) return false;
        ...
        std::FILE* stream = std::fopen(destination.c_str(), "wbx");
        if (!stream) {
            error = "cannot create local object: " + std::string(std::strerror(errno));
            return false;
        }
