...
public:
    svc_transcode::TranscodeJob job{
        "transcode-video-001", "video-001", "alice",
        "uploads/source.mp4", "uploads/transcoded/video-001.mp4",
        "PENDING", 0, 2, "", ""};
    bool available = true;
...
public:
    ...
    bool run(const svc_transcode::TranscodeJob&,
        ...
        if (!result) error = "synthetic ffmpeg failure";
        return result;
    }
};
...
bool successCase() {
    ...
    std::string error;
    return expect(worker.processOne(processed, error), "success is processed") &&
        expect(processed, "job was claimed") &&
        expect(runner.called && runner.heartbeatCalled, "runner renews lease") &&
...
bool retryCase() {
    ...
    bool processed = false;
    std::string error;
    return expect(worker.processOne(processed, error), "failure is persisted") &&
...
bool emptyQueueCase() {
    ...
    bool processed = true;
    std::string error;
    return expect(worker.processOne(processed, error), "empty queue is valid") &&
...
int main() {
    if (!successCase() || !retryCase() || !emptyQueueCase() ||
        !migrationContractCase()) return 1;
    if (!runnerBoundaryCase()) return 1;
